...
PROPORTIONAL_FONT_FAMILY = ("Arial", "Regular")
MONOSPACE_FONT_FAMILY    = ("Courier")

PROPORTIONAL_FONT_SIZE  = 12
MONOSPACE_FONT_SIZE     = 12  #a little smaller, because it it more legible at a smaller size
...
#-------------------------------------------------------------------
# mainmenu (Button box variation changed as to only display an image and buttons, no message)
#-------------------------------------------------------------------
def mainmenubox(msg = "",title=" "
    ,choices=("Button1", "Button2", "Button3")
    , image=None
    , root=None
    , msgFont = PROPORTIONAL_FONT_FAMILY
    , msgSize = PROPORTIONAL_FONT_SIZE
    , showMsg = False
    ):
    """
    Display a msg, a title, and a set of buttons.
    ...
    if showMsg == True:
        messageWidget = Message(messageFrame, text=msg, width=400)
        messageWidget.configure(font=(msgFont,msgSize))
        messageWidget.pack(side=TOP, expand=YES, fill=X, padx='3m', pady='3m')
...
def __multfillablebox(msg="Fill in values for the fields."
    ...
    for widgetIndex in range(len(fields)):
        ...
        entryWidgets[widgetIndex].configure(font=(PROPORTIONAL_FONT_FAMILY,TEXT_ENTRY_FONT_SIZE))
...
def choicebox(msg="Pick something."
    ...
    return __choicebox(msg,title,choices,buttons,firstSelected)
...
def __choicebox(msg
    ...
    choiceboxWidget.select_set(firstSelected)
...
def exceptionbox(msg=None, title=None):
    ...
    exitButton = Button(buttonsFrame, takefocus=YES, text="Restart Program", height=1, width=11)
    exitButton.pack(expand=NO, side=TOP,  padx='2m', pady='1m', ipady="1m", ipadx="2m")
    exitButton.bind("<Return>"  , __exceptionboxEXIT)
    exitButton.bind("<Button-1>", __exceptionboxEXIT)
...
def adminloansviewer(msg=""
    ...
    Also offers an "Export" option
    """

    def __loansboxEXPORT(event):
        global boxRoot, __replyButtonText
        __replyButtonText = "export"
        boxRoot.quit()
    ...
    # put the buttons in the buttonsFrame
    okButton = Button(buttonsFrame, takefocus=YES, text="OK", height=1, width=8)
    okButton.pack(expand=NO, side=TOP,  padx='2m', pady='1m', ipady="1m", ipadx="2m")
    okButton.bind("<Return>"  , __textboxOK)
    okButton.bind("<Button-1>", __textboxOK)
    okButton.bind("<Escape>"  , __textboxOK)

    exportButton = Button(buttonsFrame, takefocus=YES, text="Export", height=1, width=8)
    exportButton.pack(expand=NO, side=TOP,  padx='2m', pady='1m', ipady="1m", ipadx="2m")
    exportButton.bind("<Return>"  , __loansboxEXPORT)
    exportButton.bind("<Button-1>", __loansboxEXPORT)
